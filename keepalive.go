...
func (c *Conn) SetTcpUserTimeout(d time.Duration) error {
	return setTcpUsertimeout(c.fd, secs(d))
}
...
func SetKeepAlive(c net.Conn, idleTime time.Duration, count int, interval time.Duration, tcp_user_timeout time.Duration) (err error) {
	...
	if err = setTcpUsertimeout(fd, secs(interval)); err != nil {
		return err
	}
